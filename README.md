...
| 功能 | 核心价值 |
|---|---|
| 工作区 | 多主题并行探索、互不干扰，把知识分区管理 |
| 卡片知识树 | 把零散想法结构化成**可生长、可回溯**的知识树 |
| 三种 AI 扩展模式 | 把「想深 / 想广 / 想聚焦」三种思考动作交给 AI 展开 |
| 思维导图 | 一眼看清知识全貌与脉络，点击即达细节 |
| 导图 AI 扩展 | 从单点联想到**整树生长**，解放逐卡手动生成 |
| 文档自动摘要 | 长文一键变卡片，读得更快、记得更牢 |
| 术语库 | 看懂 AI 黑话、降低理解门槛，**尤其利于初学者** |
| 设置与连接 | 开箱即用、本地可控，Key 永不外泄 |
...
AI 卡片与文档里难免出现「江湖黑话」——CoT、Few-shot、微调、涌现、幻觉、agent……对初学者，这些词就是理解路上的一道道坎。术语库专门解决这件事：

- **利于初学者**：全工作区的术语自动聚合、去重计数，每一条都配一句话释义；遇到不懂的词随手点开即懂，不必跳出工作流去查资料。
- **降低理解难度**：术语在卡片树、导图、详情 Markdown 里**联动高亮**，鼠标悬停即现解释，阅读全程「所见即所解」，新手也能顺畅读懂 AI 生成的内容。
- **翻译 AI 界江湖黑话**：把 CoT、RAG、agent 这类缩写与黑话，沉淀成带中文定义的术语表，边用边积累成你自己的「AI 词典」——时间越久，门槛越低。

> 技术实现：高亮采用「最长优先」匹配避免短词截断长词；Markdown 渲染后仅包裹文本节点，不引入 XSS 面。
